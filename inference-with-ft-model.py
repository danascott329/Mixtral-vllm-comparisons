...
# Load list of new policies for which we need to estimate quality
docs_new = pd.concat(
    [pd.read_csv('summaries/wide-files/telework_nonprimary-summary-gpt-4o-wide-1.csv'),
     pd.read_csv('summaries/wide-files/telework_nonprimary-summary-gpt-4o-wide-2.csv'),
     pd.read_csv('summaries/wide-files/telework_nonprimary-summary-gpt-4o-wide-3.csv'),
     pd.read_csv('summaries/wide-files/telework_nonprimary-summary-gpt-4o-wide-4.csv')],
    ignore_index=True
)
# drop any policies that do not mention the topic of interest (correct to lowercase)
# also drop NA/NaN values
docs_new = docs_new.dropna()
docs_new = docs_new[docs_new[topic].str.lower().str.contains(topic.lower())]

# drop obs that say "does not mention"
docs_new = docs_new[~docs_new[topic].str.lower().str.contains("does not mention")]
docs_new = docs_new[~docs_new[topic].str.lower().str.contains("does not specify")]
docs_new = docs_new[~docs_new[topic].str.lower().str.contains("does not provide")]
docs_new = docs_new[~docs_new[topic].str.lower().str.contains("not mentioned")]
docs_new = docs_new[~docs_new[topic].str.lower().str.contains("not specified")]

# Load list of well-estimated policies for comparison
# Load list of potential policies to compare
docs = pd.concat(
    [pd.read_csv('summaries/wide-files/telework_pilot_summary-mixtral-finetuned_wide_v0320.csv'), 
     pd.read_csv('summaries/wide-files/telework_pilot_summary-mixtral-finetuned_wide_v2_v0320.csv'),
     pd.read_csv('summaries/wide-files/telework_pilot_summary-mixtral-finetuned_wide_v3_v0320.csv'),
     pd.read_csv('summaries/wide-files/telework_pilot_summary-mixtral-finetuned_wide_v4_v0320.csv'),
     pd.read_csv('summaries/wide-files/telework_pilot_summary-mixtral-finetuned_wide_v5_v0320.csv'),
     pd.read_csv('summaries/wide-files/telework_pilot_summary-mixtral-finetuned_wide_v6_v0320.csv'),
     pd.read_csv('summaries/wide-files/telework_pilot_summary-mixtral-finetuned_wide_v7_v0320.csv'),
     pd.read_csv('summaries/wide-files/telework_pilot_summary-mixtral-finetuned_wide_v8_v0320.csv'),
     pd.read_csv('summaries/wide-files/telework_pilot_summary-mixtral-finetuned_wide_v9_v0320.csv'),
     pd.read_csv('summaries/wide-files/telework_pilot_summary-mixtral-finetuned_wide_v10_v0320.csv')], 
     ignore_index=True
     )

# drop any policies that do not mention the topic of interest (correct to lowercase)
docs = docs.dropna()
docs = docs[docs[topic].str.lower().str.contains(topic.lower())]

# drop obs that say "does not mention"
docs = docs[~docs[topic].str.lower().str.contains("does not mention")]
docs = docs[~docs[topic].str.lower().str.contains("does not specify")]
docs = docs[~docs[topic].str.lower().str.contains("does not provide")]
docs = docs[~docs[topic].str.lower().str.contains("not mentioned")]
docs = docs[~docs[topic].str.lower().str.contains("not specified")]
...
# select 2 policies, 1 from the new list and 1 from the old list
def generate_pairs_with_indices(docs, docs_new, num_pairs):
    pairs = pd.DataFrame({'doc1': [], 'doc2': [], 'policy1': [], 'policy2': []})
    for i in range(num_pairs):
        samp = docs.sample(n=1)
        doc1 = samp['doc'].iloc[0]
        policy1 = samp[topic].iloc[0]
        samp = docs_new.sample(n=1)
        doc2 = samp['doc'].iloc[0]
        policy2 = samp[topic].iloc[0]
...
if task == 'compare':
    ...
    df = generate_pairs_with_indices(docs, 5000)
    print(df)

    df['output'] = df.apply(lambda row: policy_compare(row['policy1'],row['policy2']), axis = 1)
    print(df)
    filename = 'wide-files/'+ topic + '_compare_combined_mixtral_w_nonprimary_vllm.csv'
